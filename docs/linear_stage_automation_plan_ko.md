...
이 문서는 `P_sensor`에 리니어 스테이지 제어를 포함한 변위 단계별 저항값 취득 자동화를 정리한 계획 문서다. 현재 기준 하드웨어는 `SIGMAKOKI SGSP20-85`와 `SIGMAKOKI SHOT-102`로 고정하되, 상위 구조는 계속 장비 독립적으로 유지한다.
...
다만 현재 우선 구현 대상은 "리니어 스테이지 드라이버"가 아니라, 위 시나리오를 순서대로 실행하는 `오케스트레이션 계층`이다. 실제 모션 제어는 나중에 붙이되, 지금은 누르는 구간에서만 DAQ를 작동시키는 실행 흐름을 먼저 고정한다.

## 3. 현재 전제와 제약

- 기존 프로그램은 DAQ 중심 GUI와 취득 컨트롤러 구조를 이미 갖고 있다.
- 단일 DAQ 측정 루프는 다른 프로젝트에서도 재사용할 가능성이 높다.
- 리니어 스테이지 하드웨어는 `SIGMAKOKI SGSP20-85`, 컨트롤러는 `SIGMAKOKI SHOT-102`를 기준으로 한다.
- 스테이지 제어 통신은 `SHOT-102`의 `RS-232C` 호스트 제어를 기준으로 한다.
- 힘 값이 스테이지 내장 피드백인지, 별도 센서 기반인지도 아직 확정되지 않았다.
- 따라서 지금 단계에서는 "자동화 로직"과 "장비 제어 구현"을 강하게 분리해야 한다.

현재 하드웨어 전제에서 중요한 제약은 다음과 같다.

- `SGSP20-85`는 별도 원점 센서가 없고 제한 센서를 원점 검출에 사용한다.
- 변위 명령은 실제로는 펄스 단위 명령으로 내려가므로 `mm -> pulse` 환산 설정이 필요하다.
- `SHOT-102`는 `RS-232C`에서 baudrate, delimiter, flow control 설정을 맞춰야 한다.
...
현재 기준 구현은 `SIGMAKOKI SHOT-102` 어댑터를 먼저 제공하되, 상위 오케스트레이션은 계속 장비 독립 인터페이스를 사용한다.
...
스테이지 드라이버는 이미 `SHOT-102` 기준으로 1차 구현되어 있다. 다음 단계에서는 어댑터 존재 자체보다, 상위 오케스트레이션과 안전 계층을 더 분리하는 편이 적절하다.

초기 권장 방향:

- 실제 `SHOT-102` 어댑터를 구현한다.
...
이 흐름의 장점은 실제 스테이지 드라이버가 없어도 오케스트레이터와 측정 집계 로직을 먼저 검증할 수 있다는 점이다.

## 8. 오케스트레이션 우선 구현 전략

리니어 스테이지 드라이버보다 먼저 작성할 코드는 다음 순서가 적절하다.
...
이 계층은 나중에 실제 스테이지 SDK와 연결되더라도, 지금은 더미 구현이나 수동 실행 훅으로 대체 가능하다.
...
- 시나리오 러너 구현
- 측정 서비스 구현
- `SHOT-102` 명령 브리지와 시뮬레이션/대체 브리지 정리
- 레시피 실행 러너 구현
...
- `SIGMAKOKI SHOT-102` 실기 연동 검증
- `SGSP20-85` 홈, 이동, 정지, 위치 피드백 검증
- 장비 오류 메시지 표준화
