...
from p_sensor.config import (
    DEFAULT_COLORS,
    DEFAULT_CONFIG_PATH,
    DEFAULT_NI_DEVICE_NAME,
    DEFAULT_BRIDGE_REFERENCE_RESISTANCE_OHM,
    DEFAULT_EXPORT_DIRECTORY,
    DEFAULT_SENSOR_EXCITATION_VOLTAGE,
    DEFAULT_SENSOR_NOMINAL_RESISTANCE_OHM,
    build_channel_name,
...
class MainWindow(QMainWindow):
    ...
    def _build_operation_bar(self) -> QWidget:
        ...
        self.zero_button = QPushButton("Zero Cal")
        self.zero_button.setObjectName("secondaryButton")
        self.zero_button.clicked.connect(self._calibrate_zero_offset)
        ...
        layout.addWidget(self.zero_button, 0)
    ...
    def _build_channel_panel(self) -> QWidget:
        ...
        self.ni_device_name_edit = QLineEdit()
        ...
        runtime_grid.addWidget(self._create_field_label("NI Device"), 0, 1)
        runtime_grid.addWidget(self.backend_combo, 1, 0)
        runtime_grid.addWidget(self.ni_device_name_edit, 1, 1)
        runtime_grid.addWidget(self._create_field_label("Export"), 2, 0, 1, 2)
        runtime_grid.addWidget(self.export_path_edit, 3, 0, 1, 2)
        runtime_grid.addWidget(self._create_field_label("Acquisition Hz"), 4, 0)
        runtime_grid.addWidget(self._create_field_label("Display Hz"), 4, 1)
        runtime_grid.addWidget(self.acquisition_hz_edit, 5, 0)
        runtime_grid.addWidget(self.display_hz_edit, 5, 1)
    ...
    def _load_config_into_widgets(self, config: AppConfig) -> None:
        ...
        self.ni_device_name_edit.setText(config.ni_device_name)
    ...
    def _apply_ui_config(self) -> bool:
        ...
        if self.controller is not None:
            ...
            self._log("Backend disconnected after configuration change")
    ...
    def _config_from_ui(self) -> AppConfig:
        ...
        ni_device_name = self.ni_device_name_edit.text().strip() or DEFAULT_NI_DEVICE_NAME
                ...
                channels.append(
                    ChannelConfig(
                        ...
                        excitation_voltage=(
                            ...
                            if detail is not None
                            else previous_channel.excitation_voltage
                            if previous_channel
                            else DEFAULT_SENSOR_EXCITATION_VOLTAGE
                        ),
                        nominal_resistance_ohm=(
                            detail["nominal_resistance_ohm"]
                            if detail is not None
                            else previous_channel.nominal_resistance_ohm
                            if previous_channel
                            else DEFAULT_SENSOR_NOMINAL_RESISTANCE_OHM
                        ),
                        bridge_reference_resistance_ohm=(
                            detail["bridge_reference_resistance_ohm"]
                            if detail is not None
                            else previous_channel.bridge_reference_resistance_ohm
                            if previous_channel
                            else DEFAULT_BRIDGE_REFERENCE_RESISTANCE_OHM
                        ),
    ...
    def _set_channel_detail_row(
        ...
        *,
        ...
        excitation_edit = QLineEdit(
            f"{channel.excitation_voltage:.6g}" if channel else f"{DEFAULT_SENSOR_EXCITATION_VOLTAGE:.6g}"
        )
        excitation_edit.setAlignment(Qt.AlignCenter)
        self.channel_detail_table.setCellWidget(row, 3, excitation_edit)

        nominal_edit = QLineEdit(
            f"{channel.nominal_resistance_ohm:.6g}" if channel else f"{DEFAULT_SENSOR_NOMINAL_RESISTANCE_OHM:.6g}"
        )
        nominal_edit.setAlignment(Qt.AlignCenter)
        self.channel_detail_table.setCellWidget(row, 4, nominal_edit)

        bridge_reference_edit = QLineEdit(
            f"{channel.bridge_reference_resistance_ohm:.6g}"
            if channel
            else f"{DEFAULT_BRIDGE_REFERENCE_RESISTANCE_OHM:.6g}"
        )
        bridge_reference_edit.setAlignment(Qt.AlignCenter)
        self.channel_detail_table.setCellWidget(row, 5, bridge_reference_edit)
    ...
    def _channel_detail_map_from_ui(self) -> dict[tuple[int, int], dict[str, float | str]]:
        ...
        for row in range(self.channel_detail_table.rowCount()):
            ...
            details[(module_number, sensor_port)] = {
                ...
                "bridge_reference_resistance_ohm": self._detail_table_widget_float(
                    row,
                    5,
                    field_name="Bridge reference resistance",
                ),
            }
        return details
    ...
    def _refresh_runtime_summary(self) -> None:
        ...
        if backend_text == "NI":
            self.backend_label.setText(f"Backend {backend_text} {self.config.ni_device_name}")
        else:
    ...
    def _update_runtime_controls(self) -> None:
        ...
        self.ni_device_name_edit.setEnabled(not settings_locked)
        ...
        self.zero_button.setEnabled(controller_exists and not is_running)
    ...
    def _calibrate_zero_offset(self) -> None:
        try:
            ...
            sample = self.controller.backend.read(0.0)
            if not sample.readings:
                raise RuntimeError("No active channel readings were returned for zero calibration.")

            calibrated_readings: dict[int, ChannelReading] = {}
            for reading in sample.readings:
                channel = self.config.channels[reading.channel_index]
                channel.zero_offset += channel.nominal_resistance_ohm - reading.resistance_ohm
                calibrated_readings[reading.channel_index] = ChannelReading(
                    channel_index=reading.channel_index,
                    channel_name=reading.channel_name,
                    voltage=reading.voltage,
                    resistance_ohm=channel.nominal_resistance_ohm,
                    status="normal",
                )

            self._reset_history()
            self.latest_readings.update(calibrated_readings)
            self._refresh_displays()
            message = f"Zero calibration applied to {len(calibrated_readings)} channel(s)"
            self._log(message)
            self.statusBar().showMessage(message, 5000)
        except Exception as exc:  # pragma: no cover - GUI path
            self._show_error("Calibration Failed", str(exc))
